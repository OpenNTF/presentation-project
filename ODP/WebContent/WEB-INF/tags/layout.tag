...
trimDirectiveWhitespaces="true" %> <%@taglib prefix="t"
...
<meta name="turbolinks-root"
	content="${pageContext.request.contextPath}/xsp/app" />
...
<link rel="alternate"
	href="${urlBean.requestUri.resolve('xsp/app/feed.xml')}"
	type="application/rss+xml"
	title="${fn:escapeXml(translation.feedRssBlog)}">
<link rel="EditURI" type="application/rsd+xml"
	href="${urlBean.requestUri.resolve('xsp/app/rsd.xml')}" />
...
<body>
	<header id="pageheader">
		<a href="${pageContext.request.contextPath}/xsp/app/" class="logo">
			<img src="img/openntf_222222_bg.jpg"
			alt="${fn:escapeXml(translation.appTitle)}" />
		</a>
		...
		<div class="user-info">
			<c:if test="${pageContext.request.remoteUser eq 'Anonymous'}">
				<a
					href="${pageContext.request.contextPath}?Login&RedirectTo=${pageContext.request.contextPath}/xsp/app">Log
					In</a>
			</c:if>
	...
	<main id="pagebody"> <c:if test="${not empty redirectMessages}">
		<ul>
			<c:forEach items="${redirectMessages}" var="message">
				<li>${message}</li>
			</c:forEach>
		</ul>
	</c:if> <jsp:doBody /> </main>
